      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs text-slate-400">Win-Back Reports</span>
          </div>
          <h1 className="text-2xl font-bold text-slate-900">Send a Win-Back Report</h1>
        ...
        {/* Compose form */}
        <div className="bg-white rounded-2xl border border-page-border p-6 mb-6">
          <h2 className="text-sm font-semibold text-slate-900 mb-5 flex items-center gap-2">
            <span className="w-6 h-6 bg-nav-bg rounded-lg flex items-center justify-center text-white text-xs">1</span>
            Compose Report
          </h2>
          ...
          <form onSubmit={handleGenerate} className="space-y-5">
            ...
            {/* Target type */}
            <div>
              <label className="block text-xs font-semibold text-slate-600 uppercase tracking-wider mb-2">Who is this for?</label>
                ...
                {TARGET_TYPES.map(t => (
                  <button
                    ...
                    className={`p-3 rounded-xl border text-left transition-all ${
                      targetType === t.value
                        ? 'border-nav-bg bg-page-bg ring-1 ring-nav-bg'
                        : 'border-[#e2e8f0] hover:border-page-border'
                    }`}
                  >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[#f8fafc] border border-[#e2e8f0] rounded-xl px-3 py-2.5 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-nav-bg focus:ring-1 focus:ring-nav-bg"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-600 uppercase tracking-wider mb-1.5">
                  Their Email <span className="text-slate-400 font-normal normal-case">(optional, for records)</span>
                </label>
                <input
                  type="email"
            ...
            <div className="bg-[#f8fafc] border border-[#e2e8f0] rounded-xl p-3 flex items-start gap-2.5">
              ...
              <div>
                <p className="text-xs font-semibold text-slate-900 mb-0.5">AI generates this from your latest Interim Report</p>
                <p className="text-xs text-slate-500">Top findings, revenue at risk, and your site data are used to write a personalized teaser. ~30 seconds.</p>
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={generating}
              className="w-full bg-nav-bg hover:bg-nav-border disabled:opacity-60 text-white font-semibold py-3 px-6 rounded-xl transition-colors flex items-center justify-center gap-2 text-sm"
            >
              {generating ? (
        ...
        {result && (
          <div className="bg-[#f0fdf4] border border-[#bbf7d0] rounded-2xl p-6 mb-6">
            ...
            <div className="bg-white border border-[#bbf7d0] rounded-xl p-3 flex items-center gap-3 mb-3">
              <p className="text-sm text-slate-900 flex-1 font-mono truncate">{result.reportUrl}</p>
              <button
                onClick={() => copyUrl(result.reportUrl)}
                className="flex-shrink-0 bg-nav-bg text-white text-xs font-semibold px-3 py-1.5 rounded-lg hover:bg-nav-border transition-colors"
              >
                {copied ? '✓ Copied' : 'Copy'}
              </button>
            </div>
        ...
        {/* Sent reports history */}
        <div className="bg-white rounded-2xl border border-page-border p-6">
          <h2 className="text-sm font-semibold text-slate-900 mb-4 flex items-center gap-2">
            <span className="w-6 h-6 bg-nav-bg rounded-lg flex items-center justify-center text-white text-xs">2</span>
            Sent Reports
          </h2>

          {loadingHistory ? (
            <div className="py-8 flex justify-center">
              <div className="w-5 h-5 border-2 border-page-border border-t-nav-bg rounded-full animate-spin" />
            </div>
          ) : sentReports.length === 0 ? (
            <div className="py-8 text-center">
              <p className="text-sm text-slate-400">No win-back reports sent yet.</p>
              <p className="text-xs text-slate-400 mt-1">Generate your first one above — it takes ~30 seconds.</p>
            </div>
          ) : (
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                            ...
                            'bg-page-bg text-[#0891b2]'
                          }`}>
                            {isUnlocked ? '✓ Unlocked' : isExpired ? 'Expired' : 'Active'}
                          </span>
                          <span className="text-[10px] text-slate-400">{meta.targetType ?? 'PROSPECT'}</span>
                        </div>
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <button
                          ...
                          className="text-xs font-medium text-[#0891b2] hover:text-nav-bg transition-colors"
                        >
                          Copy link
                        </button>
                        {!isUnlocked && !isExpired && (
                          <button
                            ...
                            className="text-xs font-medium bg-nav-bg text-white px-2.5 py-1 rounded-lg hover:bg-nav-border transition-colors"
                          >
                            Unlock
                          </button>
                        )}
