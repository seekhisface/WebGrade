<!-- Role-specific view for Builder — excludes: Test conventions, Open questions. Re-run ingest.sh to update. -->
...
Next.js App Router — no global middleware.ts file. Auth guards are per-route:

1. **Dashboard layout** (`src/app/dashboard/layout.tsx`): calls `requireSession()` → if no JWT, redirects `/login`. Renders `<AppNav>` + `<HealthBanner>`.
2. **API routes**: each route calls `getServerSession(authOptions)` or `requireSession()` directly. No shared middleware.
3. **Ingest route**: rate limiting via `checkRateLimit()` before body parsing. No auth (by design — snippet runs on customer sites).
4. **Super admin routes**: check `session.user.email` against `SUPER_ADMIN_EMAILS` via `checkSuperAdmin()`.
